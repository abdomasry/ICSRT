...
const SocialMediaManagement = () => {
  ...
  console.log('🔄 Fetching social links via api client...');
  const data = await api.get('/api/social-links');
  const linksArray = Array.isArray(data) ? data : [];
  setSocialLinks(linksArray);
  console.log(`✅ Successfully loaded ${linksArray.length} social links`);
    } catch (error) {
  ...
  const toggleEnabled = async (link) => {
    try {
  console.log(`🔄 Toggling social link ${(link._id || link.id)} enabled status from ${link.enabled} to ${!link.enabled}`);
  ...
  await api.put(`/api/social-links/${link._id || link.id}`, updateData);
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {socialLinks?.map((link) => (
                <div key={link?._id || link?.id || Math.random()} className="group bg-white/60 backdrop-blur-sm rounded-xl border border-white/40 shadow-sm hover:shadow-lg transform hover:scale-[1.02] transition-all duration-300 p-6 relative overflow-hidden">
