...
const ServiceOrders = () => {
  ...
  const updatePriceAndNotify = async () => {
    ...
    try {
      ...
      const body = { newPrice: parseFloat(newPrice), reason: priceReason.trim(), channels, customMessage: priceCustomMessage };
                ...
                {filteredOrders.map((order) => (
                  <div key={order._id} className="flex items-center justify-between bg-white rounded-lg border border-gray-200 p-3 hover:bg-gray-50 transition">
                    ...
                    <div className="hidden md:flex items-center gap-2">
                      ...
                      {order.price && (
                        <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-700">${order.price}</span>
                      )}
                    </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50/80 rounded-lg p-4">
                      ...
                      {selectedOrder.price && <p><strong>Price:</strong> ${selectedOrder.price}</p>}
                    </div>
                  </div>
                  <div className="bg-gray-50/80 rounded-lg p-4">
              ...
              Target used: { normalizePhoneLocal(phoneOverride?.trim() || selectedOrder?.customerInfo?.whatsapp || selectedOrder?.customerInfo?.phone || selectedOrder?.phone) || 'none' }
                  ...
                  <div className="bg-gray-50/80 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        onClick={() => setPriceEditorOpen(v => !v)}
                    ...
                    {priceEditorOpen && (
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div>
                            <label className="text-sm font-medium text-gray-700">New price (USD)</label>
                            <input
                              type="number"
                              min="0"
                              value={newPrice}
