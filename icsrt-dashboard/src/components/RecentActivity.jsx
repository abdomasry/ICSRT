...
const RecentActivity = () => {
  ...
  serviceOrders: serviceOrders?.data || serviceOrders || [],
  news: news?.data || news || []
      });
    } catch (error) {
  ...
  const ActivityItem = ({ icon, title, subtitle, date, color, status }) => (
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
      ...
      <div className="space-y-4 max-h-96 overflow-y-auto">
        {/* Recent Users */}
        {recentData.users.slice(0, 2).map((user, index) => (
          <ActivityItem
            key={`user-${index}`}
            icon={<FaUsers />}
        ...
        {recentData.tickets.slice(0, 2).map((ticket, index) => (
          <ActivityItem
            key={`ticket-${index}`}
        ...
        {recentData.serviceOrders.slice(0, 2).map((order, index) => (
          <ActivityItem
            key={`order-${index}`}
        ...
        {/* Recent News */}
        {recentData.news.slice(0, 1).map((newsItem, index) => (
          <ActivityItem
            key={`news-${index}`}
            icon={<FaNewspaper />}
