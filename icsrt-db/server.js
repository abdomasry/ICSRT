...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    // Also allow any localhost:3000-3005
    if (/^http:\/\/localhost:30(0[0-5]|0|1|2|3|4|5)$/.test(origin)) return callback(null, true);
...
const createEmailTransporter = () => {
  ...
  if (!emailUser || !emailPass || emailUser === 'your-email@gmail.com' || emailPass === 'your-app-password-here') {
    ...
    console.log('📝 Please update .env file with real Gmail credentials');
    return null;
  }
...
const sendVerificationEmail = async (email, fullName, verificationCode, verificationToken) => {
  try {
    ...
    if (error.code === 'EAUTH') {
      console.log('🔐 Authentication failed - check EMAIL_USER and EMAIL_PASS in .env');
...
app.put('/api/admin/service-orders/:id/price', async (req, res) => {
  try {
    const database = await connectDB();
    const { id } = req.params;
    const { newPrice, reason, changedBy = 'admin' } = req.body;
    ...
    const discountApplied = originalAmount > newPrice ? originalAmount - newPrice : 0;
    ...
    const systemMessage = {
      ...
      channel: 'system',
      type: 'price-update',
      message: `Price updated from $${order.totalAmount} to $${newPrice}. Reason: ${reason}`,
      timestamp: new Date().toISOString()
    };
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 20px; text-align: center; color: #666; font-size: 14px;">
        <p>If you no longer wish to receive these emails, you can 
           <a href="${process.env.API_URL || 'http://localhost:3000'}/unsubscribe?email=${email}" style="color: #3B82F6;">unsubscribe here</a>
        </p>
      </div>
    </div>
  `;
    ...
    const content = `
      ...
      <div style="background: white; padding: 30px; border-radius: 12px; border: 1px solid #e5e7eb;">
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${link || (process.env.FRONTEND_URL || 'http://localhost:3002')}" 
