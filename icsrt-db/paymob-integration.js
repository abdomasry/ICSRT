...
class PaymobIntegration {
  ...
  generateIframeUrl(paymentKey) {
    if (!this.config.IFRAME_ID) {
      console.warn('⚠️  IFRAME_ID not set in .env file!');
      console.warn('⚠️  Please add PAYMOB_IFRAME_ID to your .env file');
      console.warn('⚠️  Get it from: https://accept.paymob.com/ > Settings > Payment Integrations');
      // Return a fallback URL that will show an error
      return null;
    }
    const iframeUrl = `https://accept.paymob.com/api/acceptance/iframes/${this.config.IFRAME_ID}?payment_token=${paymentKey}`;
    console.log('🌐 Generated payment URL:', iframeUrl.substring(0, 80) + '...');
    return iframeUrl;
  }
  ...
  async initiatePayment(orderData, billingInfo, paymentMethod = 'card') {
    try {
      ...
      if (!iframeUrl) {
        console.error('❌ Cannot generate payment URL - IFRAME_ID missing');
        return {
          success: false,
          error: 'Paymob IFRAME_ID not configured. Please add it to .env file.'
        };
      }
