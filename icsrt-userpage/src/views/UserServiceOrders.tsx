  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await api.post(
        `/api/user/service-orders/${selectedOrder._id}/messages`,
        {
          message: newMessage,
          ...
          {serviceOrders.length === 0 ? (
            ...
            <div className="grid gap-4 sm:gap-6">
              {serviceOrders.map((order) => (
                <div key={order._id} className="border rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-3">
                    <div className="flex-1">
                      <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-1">
                        {order.serviceName || 'Service Order'}
                      </h3>
                      <p className="text-gray-600 text-sm sm:text-base">Order #{order.orderNumber || order._id}</p>
                    </div>
                    <div className="flex flex-row sm:flex-col items-start sm:items-end gap-2">
                      ...
                      <span className="text-base sm:text-lg font-bold text-blue-600 ml-auto sm:ml-0">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                    <div className="space-y-2">
                      <p className="text-xs sm:text-sm text-gray-600 flex items-center">
                        ...
                        <span className="truncate">Submitted: {formatDateTime(order.submittedAt)}</span>
                      </p>
                      {order.lastMessageAt && (
                        <p className="text-xs sm:text-sm text-gray-600 flex items-center">
                          <FaComments className="mr-1 flex-shrink-0" />
                          <span className="truncate">Last message: {formatDateTime(order.lastMessageAt)}</span>
                        </p>
                      )}
                    </div>
                    <div>
                      {order.messageStats && (
                        <p className="text-xs sm:text-sm text-gray-600 flex items-center">
                          <FaComments className="mr-1 flex-shrink-0" />
                          <span>Messages: {order.messageStats.total}</span>
                  ...
                  {order.description && (
                    <p className="text-gray-700 mb-4 text-sm sm:text-base line-clamp-2">{order.description}</p>
                  )}
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                    ...
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-lg transition-colors flex items-center justify-center gap-2 text-sm sm:text-base w-full sm:w-auto"
                    >
                      <FaEye /> View Details
                    </button>
                  </div>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 sm:p-6">
              <div className="flex justify-between items-start">
                <div className="flex-1 pr-4">
                  <h2 className="text-lg sm:text-2xl font-bold truncate">{selectedOrder.serviceName}</h2>
                  <p className="text-blue-100 text-sm sm:text-base">Order #{selectedOrder.orderNumber || selectedOrder._id}</p>
                </div>
                <button
                  onClick={closeModal}
                  className="text-white hover:text-gray-300 text-xl sm:text-2xl flex-shrink-0"
                >
                  ×
                </button>
              </div>
            ...
            <div className="flex border-b bg-gray-50">
              <button
                ...
                className={`flex-1 px-3 sm:px-6 py-3 font-medium text-sm sm:text-base ${
                  activeTab === 'details'
                    ? 'text-blue-600 border-b-2 border-blue-600 bg-white'
                    : 'text-gray-600 hover:text-gray-800'
                }`}
              >
                <FaTag className="inline mr-1 sm:mr-2" /> <span className="hidden sm:inline">Order </span>Details
              </button>
              <button
              ...
              >
                <FaComments className="inline mr-1 sm:mr-2" /> 
                <span className="hidden sm:inline">Conversation</span>
                <span className="sm:hidden">Chat</span>
                {selectedOrder.messageStats && selectedOrder.messageStats.unread > 0 && (
                  <span className="ml-1 sm:ml-2 bg-red-500 text-white text-xs px-1 sm:px-2 py-1 rounded-full">
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        <p className="text-sm sm:text-base"><strong>Service:</strong> {selectedOrder.serviceName}</p>
                        <p className="text-sm sm:text-base"><strong>Status:</strong> 
                          <span className={`ml-2 px-2 py-1 rounded text-xs sm:text-sm ${getStatusColor(selectedOrder.status)}`}>
                            {getStatusIcon(selectedOrder.status)} {selectedOrder.status}
                          </span>
                        </p>
                        <p className="text-sm sm:text-base"><strong>Amount:</strong> <span className="text-green-600 font-semibold">${selectedOrder.totalAmount || 0}</span></p>
                        <p className="text-sm sm:text-base"><strong>Submitted:</strong> {formatDateTime(selectedOrder.submittedAt)}</p>
                        {selectedOrder.priceUpdatedAt && (
                          <p className="text-sm sm:text-base"><strong>Price Updated:</strong> {formatDateTime(selectedOrder.priceUpdatedAt)}</p>
                        )}
                      </div>
                    </div>
                    
                    <div>
                      <h3 className="text-base sm:text-lg font-semibold mb-3">Contact Information</h3>
                      <div className="space-y-2">
                        <p className="text-sm sm:text-base break-all"><strong>Email:</strong> {selectedOrder.userEmail || user.email}</p>
                        {selectedOrder.customerInfo && (
                          <>
                            <p className="text-sm sm:text-base"><strong>Name:</strong> {selectedOrder.customerInfo.name}</p>
                            <p className="text-sm sm:text-base"><strong>Phone:</strong> {selectedOrder.customerInfo.phone}</p>
                            <p className="text-sm sm:text-base"><strong>Country:</strong> {selectedOrder.customerInfo.country}</p>
                            <p className="text-sm sm:text-base"><strong>Organization:</strong> {selectedOrder.customerInfo.organization}</p>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  {selectedOrder.description && (
                    <div>
                      ...
                      <p className="text-gray-700 bg-gray-50 p-3 sm:p-4 rounded-lg text-sm sm:text-base">{selectedOrder.description}</p>
                    </div>
                  )}
