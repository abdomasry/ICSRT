    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white shadow rounded-xl p-8 border border-gray-100 text-center max-w-md">
          ...
          <div className="text-lg font-semibold mb-2">Please log in to view your payments</div>
          <a href="/login" className="inline-block mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg">Login</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-4 md:p-8">
      <div className="bg-white/80 rounded-xl border border-white/20 shadow-xl p-6 md:p-8 mb-6">
        <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">💳 Payments</h1>
        <p className="text-gray-600 mt-2">View your payment history and statuses</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white/80 rounded-xl border border-white/20 shadow p-8 text-center">
          ...
          <div className="text-gray-700 font-medium">No payments yet</div>
        </div>
      ) : (
        <div className="bg-white/80 rounded-xl border border-white/20 shadow overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 border-t border-gray-100">
            ...
            <div className="flex gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="px-3 py-1.5 rounded bg-gray-100 disabled:opacity-50">Prev</button>
              <span className="text-sm text-gray-700">Page {page} / {totalPages}</span>
              <button disabled={page === totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="px-3 py-1.5 rounded bg-gray-100 disabled:opacity-50">Next</button>
            </div>
          </div>
        </div>
      )}
