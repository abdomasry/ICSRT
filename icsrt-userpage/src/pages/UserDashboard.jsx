...
const UserDashboard = () => {
  ...
  if (stats) {
        // Accept zero counts; overwrite defaults
        const safe = {
          tickets: Number(stats.tickets || 0),
          serviceOrders: Number(stats.serviceOrders || 0)
        };
        setUserStats(safe);
  ...
  if (orders) {
      ...
      console.log('Dashboard: Service orders fetch failed');
    }
  };
                          ...
                          <div className={`flex items-center space-x-2 mt-1 ${isRTL ? 'space-x-reverse justify-end' : ''}`}>
                            ...
                            {order.messages && order.messages.length > 0 && (
                              <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">
                                {order.messages.length} {t('common.messages')}
                              </span>
                            )}
                          </div>
                        </div>
  ...
  {showServiceOrderModal && selectedServiceOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto ${isRTL ? 'rtl' : 'ltr'}`}>
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <div className={`flex justify-between items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
                <h2 className={`text-xl font-bold text-gray-900 dark:text-white ${isRTL ? 'text-right' : 'text-left'}`}>{t('dashboard.order.service')} {t('common.details')}</h2>
            ...
            <div className="p-6">
              <div className="space-y-4">
                <div>
                  <h3 className={`font-semibold text-gray-900 dark:text-white ${isRTL ? 'text-right' : 'text-left'}`}>{t('profile.orderInformation')}</h3>
                  <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.order.number')}</p>
                      <p className="font-medium text-gray-900 dark:text-white">{selectedServiceOrder.orderNumber}</p>
                    </div>
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.order.service')}</p>
                      <p className="font-medium text-gray-900 dark:text-white">{selectedServiceOrder.serviceName}</p>
                    </div>
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.order.status')}</p>
                    ...
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.order.date')}</p>
                ...
                {selectedServiceOrder.totalAmount && (
                  <div className={isRTL ? 'text-right' : 'text-left'}>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{t('dashboard.order.amount')}</p>
                ...
                {selectedServiceOrder.details && (
                  <div>
                    <h3 className={`font-semibold text-gray-900 dark:text-white ${isRTL ? 'text-right' : 'text-left'}`}>{t('common.details')}</h3>
                    <div className="mt-2 bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                      {Object.entries(selectedServiceOrder.details).map(([key, value]) => (
                        <div key={key} className={`mb-2 ${isRTL ? 'text-right' : 'text-left'}`}>
